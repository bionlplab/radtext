...
import ptakes.utils
from ptakes.neg import ngrex
from ptakes.neg.constants import NEGATION, UNCERTAINTY, NegPattern, NegResult
from ptakes.neg.ngrex import NgrexMatch, NgrexPattern
...
def find_nodes(graph, begin, end):
    for node in graph.nodes():
        if ptakes.utils.intersect((begin, end), (graph.nodes[node]['start'], graph.nodes[node]['end'])):
            yield node
